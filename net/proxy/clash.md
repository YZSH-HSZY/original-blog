...
[clash说明文档](https://a76yyyy.github.io/clash/zh_CN/configuration/dns.html)

## clash 工作方式

- Inbound 入站
> Inbound 是在本地端监听的部分, 通过打开一个本地端口并监听传入的连接. 当连接进来时, Clash 会查询配置文件中配置的规则, 并决定连接应该去哪个 Outbound 出站.
- Outbound 出站
...
> 代理部分 (proxies)
- name：代理的名称，用于在规则中引用。
- type：代理的类型，例如 socks5、http 等。
- server：代理服务器的地址（这里是本地 IP 127.0.0.1）。
- port：代理服务器的端口（这里是 1080）。
- skip-cert-verify：是否跳过证书验证（可选）。

> 规则部分 (rules)
- IP-CIDR：用于匹配 IP 地址或网段。格式：`IP-CIDR,<IP地址>/<子网掩码>,<代理名称>`， 如 `IP-CIDR,192.168.1.100/32,Local Proxy` 表示将 192.168.1.100 的流量路由到 Local Proxy
- MATCH：默认规则，用于处理未匹配的流量。如：`MATCH,DIRECT` 表示其他流量直连
...
proxies:
  ...
  - name: "Local Proxy"  # 代理名称
    type: socks5         # 代理类型（如 socks5、http）
    server: 127.0.0.1    # 代理服务器地址
    port: 1080           # 代理端口
    skip-cert-verify: true  # 跳过证书验证（可选）
...
rules:
  ...
  - IP-CIDR,127.0.0.0/8,DIRECT  # 将 127.0.0.0/8 网段的流量路由到 DIRECT
  - GEOIP,CN,DIRECT
  - DST-PORT,80,DIRECT
  - SRC-PORT,7777,DIRECT
  - RULE-SET,apple,REJECT # 仅 Premium 版本支持
  # 默认规则（可选）
  - MATCH,auto  # 其他流量使用auto测试连接
```

**注意** `clash for android` 需要在 `设置-->网络-->自动路由系统流量` 以启动代理转发